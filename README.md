...
* Level 7 alien mothership boss battle
* Boss HP system, boss attacks, and boss-phase pressure
* Score, lives, danger meter, upgrades, high score, game over, and victory states
* Sound effects and looping background music
* Cleaned retro UI with consistent border coloring and simplified bottom command text
...
│   ├── enemies/          # UFO enemy and boss enemy
│   ├── player/           # Player ship
...
│   ├── spawning/         # Asteroid, cluster, UFO, power-up, and shrapnel spawning
...
If the batch file does not run, make sure it is located in the main project folder, the same folder where `src` is located.
...
For Windows Command Prompt manual compilation, use the provided `run_windows.bat` file because it automatically collects all Java source files.

## OOP Concepts Demonstrated

### Encapsulation

Player lives, score, input state, upgrade levels, high score, UFO behavior, boss health, asteroid damage, and movement values are stored inside their respective classes and accessed through methods.

### Inheritance

`Player`, `Bullet`, `EnemyBullet`, `Asteroid`, `MeteorShard`, `UfoEnemy`, `AlienBoss`, and `PowerUp` are built around the shared object structure. Asteroid and power-up variations extend their parent classes.
...
This package is organized into clear subpackages for readability and maintainability. The final version includes the original arena shooter gameplay, the updated retro UI cleanup, and the Level 7 alien mothership boss battle. The project was compiled successfully using UTF-8 with `javac` after the final merge.
