...
public class FixedGridPanel extends JPanel {
    ...
    private Color getColorForSymbol(char symbol) {
        ...
        if (isPlayerSymbol(symbol)) return getPlayerColor();
        if (isPlayerBulletSymbol(symbol)) return getBulletColor();
        if (isEnemyBulletSymbol(symbol)) return new Color(255, 90, 90);
        ...
        if (isBossSpriteSymbol(symbol)) return new Color(255, 100, 255);
    ...
    private boolean isPlayerSymbol(char symbol) {
        return symbol == GameSymbols.PLAYER_UP
                || symbol == GameSymbols.PLAYER_DOWN
                || symbol == GameSymbols.PLAYER_LEFT
                || symbol == GameSymbols.PLAYER_RIGHT
                || symbol == GameSymbols.PLAYER_UP_LEFT
                || symbol == GameSymbols.PLAYER_UP_RIGHT
                || symbol == GameSymbols.PLAYER_DOWN_LEFT
                || symbol == GameSymbols.PLAYER_DOWN_RIGHT
                || symbol == GameSymbols.PLAYER_SHIELDED;
    }

    private boolean isPlayerBulletSymbol(char symbol) {
        return symbol == GameSymbols.BULLET_VERTICAL
                || symbol == GameSymbols.BULLET_HORIZONTAL
                || symbol == GameSymbols.BULLET_DIAGONAL_FORWARD
                || symbol == GameSymbols.BULLET_DIAGONAL_BACK;
    }

    private boolean isEnemyBulletSymbol(char symbol) {
        return symbol == GameSymbols.ENEMY_BULLET_DOWN
                || symbol == GameSymbols.ENEMY_BULLET_UP
                || symbol == GameSymbols.ENEMY_BULLET_LEFT
                || symbol == GameSymbols.ENEMY_BULLET_RIGHT;
    }

    private Color getPlayerColor() {
        int palette = (currentLevel - 1) % 4;
        if (palette == 1) return new Color(255, 255, 0);
        if (palette == 2) return new Color(255, 100, 0);
        if (palette == 3) return new Color(255, 255, 255);
        return new Color(0, 255, 255);
    }

    private Color getBulletColor() {
        int palette = (currentLevel - 1) % 4;
        if (palette == 1) return new Color(255, 0, 255);
        if (palette == 2) return new Color(50, 255, 0);
        if (palette == 3) return new Color(255, 0, 0);
        return new Color(0, 255, 255);
    }

    private boolean isBossSpriteSymbol(char symbol) {
