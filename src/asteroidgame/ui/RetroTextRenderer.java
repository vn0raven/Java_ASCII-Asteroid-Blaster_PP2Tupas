...
public class RetroTextRenderer {
    ...
    private String getOverlayText(int row, GameFrame frame) {
        ...
        if (frame.isVictory()) {
            ...
            if (row == 10) return "POWER-UPS COLLECTED: " + frame.getPowerUpsCollected();
            if (row == 12) return "FINAL BUILD FIRE " + frame.getFireRateLevel() + "  SHOT " + frame.getShotLevel() + "  SHIELD " + frame.getShieldCharges();
        ...
        if (frame.isGameOver()) {
            ...
            if (row == 11) return "POWER-UPS COLLECTED: " + frame.getPowerUpsCollected();
            if (row == 13) return "DANGER REACHED: " + frame.getDangerLevel() + " / " + frame.getDangerLimit();
            if (row == 15) return "PRESS R TO RESTART";
        }
    ...
    private String levelTip(int level) {
        if (level == 1) return "LEARN THE MOVEMENT AND SHOOTING RHYTHM";
        if (level == 2) return "WIDE ASTEROIDS ARE EASIER TO HIT";
        if (level == 3) return "HEAVY ASTEROIDS NEED THREE HITS";
        if (level == 4) return "◉ CRACKS INTO ●, THEN ◎";
        if (level == 5) return "METEOR STORM: MORE LANES OPEN";
