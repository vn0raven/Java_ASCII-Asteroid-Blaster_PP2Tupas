...
public class CollisionManager {
    ...
    public CollisionResult checkBulletBossCollision(List<Bullet> bullets, AlienBoss boss, List<Explosion> explosions) {
        int earnedPoints = 0;
        int destroyedCount = 0;
        int damagedCount = 0;

        if (boss == null || !boss.isActive()) {
            return new CollisionResult(0, 0, 0);
        }

        for (Bullet bullet : bullets) {
            if (!bullet.isActive() && bullet.getPathStepCount() == 0) continue;

            if (bulletHitsBoss(bullet, boss)) {
                bullet.setActive(false);
                boolean destroyed = boss.takeHit();

                if (destroyed) {
                    earnedPoints += boss.getDestroyScoreValue();
                    destroyedCount++;
                    explosions.add(new Explosion(boss.getX(), boss.getCenterY()));
                    explosions.add(new Explosion(boss.getX() - 4, boss.getCenterY()));
                    explosions.add(new Explosion(boss.getX() + 4, boss.getCenterY()));
                    explosions.add(new Explosion(boss.getX(), boss.getCenterY() + 2));
                } else {
                    earnedPoints += boss.getHitScoreValue();
                    damagedCount++;
    ...
    private boolean bulletHitsBoss(Bullet bullet, AlienBoss boss) {
        int steps = Math.max(1, bullet.getPathStepCount());

        for (int step = 1; step <= steps; step++) {
            if (boss.isHitBy(bullet.getPathX(step), bullet.getPathY(step))) {
                return true;
            }
        }

        return boss.isHitBy(bullet.getX(), bullet.getY());
    }
    ...
    public boolean checkPlayerBossCollision(Player player, AlienBoss boss) {
        if (boss == null || !boss.isActive()) {
            return false;
        }

        return boss.overlapsPoint(player.getX(), player.getY());
    }
